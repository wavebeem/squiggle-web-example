...
(function () {
    ...
    var sqgl$$object = function (data) {
        if (!sqgl$$isArray(data)) {
            throw new sqgl$$Error('objects can only be constructed from an array');
        }
        var obj = {};
        var i = 0;
        var n = data.length;
        while (i < n) {
            if (typeof data[i][0] !== 'string') {
                throw new sqgl$$Error('object keys must be strings: ' + data[i]);
            }
    ...
    var sqgl$$isObject = function (x) {
        if (arguments.length !== 1) {
            throw new sqgl$$Error('wrong number of arguments to sqgl$$isObject');
        }
    ...
    return function () {
        ...
        var Object = sqgl$$get('Object', global);
        ...
        var start = sqgl$$freeze(function () {
            if (arguments.length !== 0) {
                throw new sqgl$$Error('expected 0 argument(s), got ' + arguments.length);
            }
            return function () {
                set('state', sqgl$$object([[
                        'count',
                        0
                    ]]), app);
                set('tree', render(sqgl$$get('state', app)), app);
                set('root', createElement(sqgl$$get('tree', app)), app);
                sqgl$$methodCall('appendChild', sqgl$$get('body', document), sqgl$$freeze([sqgl$$get('root', app)]));
                return setInterval(update, 1000);
            }();
        });
        var update = sqgl$$freeze(function () {
            if (arguments.length !== 0) {
                throw new sqgl$$Error('expected 0 argument(s), got ' + arguments.length);
            }
            return function () {
                var newState = $tilde(sqgl$$get('state', app), sqgl$$object([[
                        'count',
                        $plus(sqgl$$get('count', sqgl$$get('state', app)), 1)
                    ]]));
                var newTree = render(newState);
                var patches = diff(sqgl$$get('tree', app), newTree);
